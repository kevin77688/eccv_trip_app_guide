...
- 使用者在每台裝置匯入 `.enc` 檔，通過雜湊比對後儲存於本機 IndexedDB，出示時仍需密碼。公開網站與 CI 產出的 APK 不附帶私人票券；原始檔保留在本機。
- 加密票券停止受 Git 追蹤，姓名、訂單 PIN 與訂位識別資料已遮蔽。這取代下方歷史紀錄中上傳 `.enc` 的做法，不改寫既有 Git 歷史。重新加密時會同步更新匯入檢查碼，裝置需匯入新檔。
