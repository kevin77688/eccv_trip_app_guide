...
- **伴手禮採買指南客觀化**：
  - 剔除宣傳式誇飾詞（如「精品第一品牌」、「奢華感」、「朝聖標竿」、「不可多得的硬核工藝逸品」、「國寶巧克力」、「完全零踩雷風險」），改為平實、客觀的產品特色、烘焙風味、材質工藝與適用送禮情境說明。
  - 行李伴手禮頁導覽簡化，移除報幕式宣告與過度包裝詞彙，清楚聚焦「一人一份主題禮包」與「辦公室公用桌分享零食」。
- **景點快速查找（Places）介紹去贅詞**：
  - 凡爾賽宮修正為「法國 17 世紀宮殿建築」；互動科學中心去除「探索體驗」疊字贅語，明確提示展區範圍較大；特羅卡德羅與雪鐵龍公園熱氣球去除「最佳視角」與「全球最大」等評價誇大標籤，回歸實際設施與拍攝條件描述。
- **維護與更新介面文字精簡**：
  - 去除填充贅詞與過度敬稱（如「徹底清除」、「保留您的...」），將操作說明聚焦於動作與保留範圍；Android APK 離線維護說明簡化。
- **Android 原生生物辨識提示修飾**：
  - `BiometricsPlugin.java` 啟用指紋提示說明由「金鑰存放於 Samsung Knox 硬體安全晶片中，不具備密碼直接洩漏風險。」優化為「金鑰由 Samsung Knox 硬體晶片保護，不會洩漏密碼文字。」
- **全站快取升級**：PWA 快取版本提升至 `eccv-guide-v20260906-01`，全站 19 份 HTML 檔案、`site/sw.js`、`site/js/core.js` 與工具頁版本資訊同步更新。

## 手機瀏覽體驗全面最佳化：景點頁精簡清單/即時搜尋與行李頁包包分流折疊（2026-09-05）

- **景點快速查找（Places）手機瀏覽減負與雙模式切換**：
  - **精簡清單模式（Compact View）**：卡片垂直高度由原本 450px～650px 劇幅壓縮至約 68px，呈現 48px 微型圓角封面、中英雙語地名、建議停留時間、主題標籤與一鍵直通 Google Maps 導航圖示。點擊可展開/收合詳細背景、推薦理由、官方票務與外部連結，手機單屏可掃視 5～8 個景點，大幅消除長篇滑動負擔。
  - **圖文卡片模式（Cards View）**：保留原本詳盡的雙欄圖文卡片展示，供使用者在大螢幕或深入規劃時切換。支援使用者偏好記憶（localStorage）與響應式切換。
  - **即時關鍵字搜尋（Real-time Search）**：提供帶清除鍵之搜尋輸入框，支援中文名稱、英文名稱、城市（Malmö/Copenhagen/Paris/Beauvais/Versailles）、特色標籤、票務備註多維度即時過濾，並具備優雅的無結果空狀態反饋。
- **行李清單（Packing）分包篩選與手風琴折疊**：
  - **包包分類膠囊標籤（Bag Filter Pills）**：於行李工具列增設「全部 (67)」、「① 隨身小包 (11)」、「② 後背包 (26)」、「③ 行李箱 (30)」快捷分流標籤，點選即聚焦單一背包物品，免去手機長距離翻找。頂端概覽導航卡片亦支援點擊自動切換包包並平滑錨點捲動。
  - **獨立手風琴折疊（Accordion Collapse）**：三大背包區塊標題皆具備獨立點擊折疊/展開功能（搭配旋轉箭頭指示器 ▾ / ▴），工具列增設「全部收合 / 全部展開」全域切換鈕，打包時可隨心收折已確認背包，極大化行動端垂直可視範圍。
  - **動態進度同步**：分包過濾與折疊操作無縫相容於即時勾選計數、分包百分比進度條與 localStorage 本機儲存。
- **全站快取升級**：PWA 快取版本提升至 `eccv-guide-v20260905-22`，全站 19 份 HTML 檔案、`site/sw.js`、`site/js/core.js` 與工具頁版本資訊同步更新。

## 旅行翻譯介面極簡化與 S23 單畫面整合（2026-09-05）

- **API 連線面板摺疊化（Collapsible Panel）**：
  - 將過往佔據垂直高達 350px～450px 的連線設定區塊重構為原生 `<details>` / `<summary>` 摺疊面板，預設保持收合狀態，高度大幅壓縮至約 36px。
  - Summary 摘要列清楚提示當前連線狀態（如「✓ 已連線」或「尚未設定」）與「展開設定 ▾」，驗證成功後自動收合並顯示成功提示。
- **離線語言包就緒後自動隱藏**：
  - 偵測到 Android 離線翻譯模型已準備就緒（`offlineReady === true`）時，自動將佔用約 120px 的「下載離線語言包」宣傳橫幅完全隱藏，釋放珍貴螢幕空間；下載完成後彈出完成 toast 並收合，僅在設定摘要保留微型離線就緒徽章。
- **分頁模式切換（拍照・圖片 vs 輸入文字）**：
  - 於翻譯卡片頂端增設「📷 拍照・圖片」與「✍️ 輸入文字」分頁切換籤，旅行預設啟用「📷 拍照・圖片」。
  - 在拍照模式下完全隱藏大面積之 168px textarea，徹底杜絕無謂的版面浪費。
  - 文字模式下提供緊湊輸入框（~84px）與「清空」/「翻譯文字」專屬按鈕。
- **Samsung S23 手機單螢幕拍照與結果無縫整合（免滑動）**：
  - **緊湊相機後鏡頭視訊**：相機開啟時視訊高度嚴格限制在 180px～210px，底部懸浮「📸 拍攝並翻譯」與「✕ 關閉相機」，不超出首屏。
  - **拍完即呈結果**：拍攝完成瞬間鏡頭自動關閉，頂部替換為高度僅約 36px 的極簡相片縮圖資訊條（含縮圖、檔名與「📷 重新拍攝」/「更換」快捷鈕），翻譯結果卡片緊隨其下直接呈現。
  - **首屏空間預算達成**：S23 可視高度約 660px，折疊設定列（36px）+ 分頁切換（38px）+ 相片資訊條（48px）+ 翻譯結果卡片（220px）= 約 342px，拍照、看翻譯、一鍵「📋 複製」、點擊「重新拍攝」全在同一螢幕高度內一氣呵成，徹底告別來回滑動困擾。
- **全站快取升級**：PWA 快取版本提升至 `eccv-guide-v20260905-21`，全站 19 份 HTML 檔案、`site/sw.js`、`site/js/core.js` 與工具頁版本資訊同步更新。
...
- **架構隔離與中性呈現**：
  - 於 `site/packing.html` 頂部新增分頁切換（「🎒 行李打包清單」與「🎁 伴手禮推薦指南」），徹底將伴手禮採買資訊與個人行李 3 包打包清單分流，彼此進度與狀態互不干擾。
  - 文案採中性、客觀主題式包裝，適合所有同行夥伴參考，避免私人稱謂標籤。
- **精選主題風格禮包（適合一人一份，預算約 NT$1,000～1,500）**：
  - **極黑工藝・精品甘草**：丹麥 Lakrids by Bülow 巧克力甘草球雙罐組（黑金霧面罐裝，比利時巧克力裹海鹽焦糖甘草，哥本哈根中央車站或機場 CPH 免稅店購，約 220～260 DKK）。
  - **主廚工藝・傳統海鹽**：丹麥 Læsø Salt 柴燒手工海鹽禮袋（復古麻布袋封裝，結晶薄脆帶微燻香，哥本哈根 Torvehallerne 市集購，約 180～250 DKK）。
  - **療癒設計・經典木偶**：丹麥 Kay Bojesen 經典原木可動木偶／小木鳥（丹麥 1951 年經典設計，橡木或柚木打磨，哥本哈根 Strøget 專門店或百貨購，約 260～350 DKK）。
  - **世界冠軍・精品咖啡**：丹麥 The Coffee Collective 產地烘焙精品咖啡豆組（雙料世界咖啡冠軍名廠，淺焙果酸與明亮風味莊園豆，哥本哈根門市或中央車站外帶吧購，約 240～300 DKK）。
- **團隊零食分享包（適合 16 人公用桌，總預算約 NT$400～500 / 140 SEK）**：
  - **國民軟糖・老少咸宜**：瑞典 Ahlgrens Bilar 經典小車棉花糖軟糖 ×2 包（瑞典經典車型軟糖，馬爾默超市購，約 36 SEK）。
  - **皇室御用・熱量補給**：瑞典 Marabou 牛奶巧克力海鹽焦糖限定版大片裝 ×2 片（皇室認證品牌，甜鹹奶香，馬爾默超市購，約 55 SEK）。
  - **傳統 Fika・茶水間首選**：瑞典 Ballerina 雙層榛果可可夾心餅乾 ×1 筒（1960 年代經典下午茶餅乾，馬爾默超市購，約 18 SEK）。
  - **文化特色・重鹹甘草**：瑞典 Malaco Djungelvrål 鹹甘草猴子糖 ×1 包（高濃度氯化銨 Salmiak 鹹味強勁，馬爾默超市購，約 15 SEK）。
- **食品保存與託運重大警示**：
  - 瑞典牙膏狀魚卵抹醬（Kalles Kaviar）官方規格明確要求全程 2～8°C 冷藏，且 190g 超過 100ml 隨身液體限制必須託運；因 9/12 離開瑞典至 9/18 巴黎長達 7 天無法確保全程低溫，常溫攜帶必會油水分離或發酵爆管，強烈不建議作為長途伴手禮帶回台灣。建議在馬爾默 First Camp 小木屋廚房享用，或返台於 IKEA 瑞典食品超市直接購買冷藏進口品。
- **全站快取升級**：PWA 快取版本提升至 `eccv-guide-v20260905-19`，全站 19 份 HTML 檔案、`site/sw.js` 與工具頁版本資訊同步更新。
...
- **上午分流參觀與逛街**：
  - 同行者搭 M4 轉 M8 前往榮軍院軍事博物館（Musée de l’Armée，10:00 入場），參觀古代盔甲與拿破崙一世陵墓。
  - 個人搭 M4 直達左岸樂蓬馬歇百貨（Le Bon Marché），欣賞 Gustave Eiffel 工作室操刀的挑高鋼構玻璃中庭，全程吹冷氣避開熱浪，逛生活選品與 La Grande Épicerie 頂級食品館。
- **中午戰神廣場周邊全員會合**：
  - 12:30 兩組搭車至 École Militaire 站會合，於周邊法式餐館享用炙烤牛排與肉類料理，無海鮮壓力。
- **艾菲爾鐵塔四人全員合照與特羅卡德羅露台**：
  - 13:45 四人穿過戰神廣場，於鐵塔正下方仰拍鋼構力學細節（地面參觀不登塔、免排隊安檢）。
  - 四人一同穿過耶拿橋（Pont d’Iéna）登上特羅卡德羅廣場（Place du Trocadéro）高地大理石露台，拍下無遮蔽的經典鐵塔全景四人大合照。
- **比爾哈凱姆鋼構橋與天鵝島自由女神**：
  - 15:15 向西南漫步至比爾哈凱姆雙層鋼架橋（Pont de Bir-Hakeim，欣賞經典鉚接鋼橋與地鐵 6 號線高架視角）。
  - 從橋中央階梯走下天鵝島（Allée des Cygnes）林蔭道，漫步 850 公尺抵達島端，與 1889 年原版青銅自由女神像合照。
- **博格內爾商場與 Fnac 旗艦店**：
  - 16:30 過格勒納勒橋直達現代綠能鋼構商場 Beaugrenelle Paris，全員於室內吹冷氣小憩，逛大型 Fnac 旗艦店挑選 3C 科技配件與設計選品。
- **雪鐵龍公園熱氣球（Ballon de Paris Generali）**：
  - 17:30 前往雪鐵龍公園（Parc André Citroën），搭乘全球最大繫留氦氣球垂直升空至 150 公尺（約 50 層樓高），空中 360 度俯瞰巴黎全景與遠眺鐵塔（現場視天候風速購票，每人約 20 歐元）。
- **傍晚 RER C 直達碼頭與 Francette 晚餐**：
  - 18:30 從 Pont du Garigliano 搭乘 RER C 直達 Champ de Mars Eiffel Tower 站（車程僅 6 分鐘），步行 5 分鐘抵達 Port de Suffren 碼頭。
  - 19:00 享用已確認預約（4 位）之 Francette 水上景觀晚餐，近距離欣賞鐵塔點燈夜景。
- **全站快取升級**：PWA 快取版本提升至 `eccv-guide-v20260905-16`，全站 19 份 HTML 檔案、`site/sw.js` 與工具頁版本資訊同步更新。

## 凡爾賽宮全日與全站時間軸圖示風格全面升級（2026-09-05）

- **修復 09/17 凡爾賽宮時間軸圖示字樣異常**：
  - 修正原程式碼中將城堡誤傳為純文字「城堡」之排版問題，全面導入標準 Emoji 與分類色彩系統。
  - **凡爾賽主宮殿（Palace of Versailles）**：配置歐式城堡宮殿圖示 `🏰`，搭配淺紫丁香底色（`tone-sight`），突顯世界文化遺產與鏡廳歷史定位。
  - **凡爾賽花園漫步（Gardens of Versailles）**：精準分類為林蔭花園圖示 `🌳` 與湖水綠底色（`tone-nature`），不再被宮殿關鍵字覆蓋。
  - **凡爾賽左岸站回程與跨區列車**：精準關聯為列車車站圖示 `🚆` 與中性底色（`tone-transit`），符合實際乘車情境。
- **全站時間軸卡片視覺與類別色系升級（`site/css/styles.css`、`site/js/pages/day.js`）**：
  - 圖示容器升級為五大類別柔和情境底色：景點古蹟（`tone-sight`，柔和紫）、自然花園（`tone-nature`，湖水綠）、美食餐飲（`tone-food`，暖蜜金）、票券門禁（`tone-ticket`，珊瑚粉）與交通轉運（`tone-transit`，紙質灰）。
  - 加入細緻陰影與可點擊卡片微幅縮放懸浮動態（`transform: scale(1.03)`），大幅提升手機與桌機雙視口閱讀質感。
  - 順帶校正全行程 14 天各站點匹配權重，包含聖心堂（`⛪`）、聖徒禮拜堂（`⛪`）、克里斯蒂安堡宮（`🏛️`）與機場轉乘列車（`🚆`）。
- **全站快取升級**：PWA 快取版本提升至 `eccv-guide-v20260905-15`，全站 19 份 HTML 檔案、`site/sw.js` 與工具頁版本資訊同步更新。
