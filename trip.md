...
- 交通頁預設開啟「當日資訊」，依所選日期集中顯示票券、當晚住宿與航班，並提供地址複製與導航。完整分類保留，日期與分類會在本次瀏覽中記住。
- 交通頁的 9/12 寄物提醒與既定主方案一致：優先 CPH 機場，中央車站為備案。
- 行李頁預設顯示「還沒收」，記住包款、篩選及收合狀態；清单完成時會提示可切回全部核對。
- 分裝說明與登機限制移至清單下方展開查看，編輯、同步與重設集中在「編輯與備份」。重設操作使用配合明暗主題的確認視窗。
- 景點精簡清單以中文名為主、外文名為輔，放大導航及展開按鈕；搜尋文字與篩選會在本次瀏覽中保留。
- 回到頁面時恢復捲動位置與展開的補充區塊；有指定錨點時優先前往目標。鍵盤開啟時暫時隱藏底部導覽，避免遮住輸入區。
- 更新維護頁會實際檢查行程快取、裝置票券與 Android 翻譯語言包，不把網路連線狀態當作離線準備完成。PWA 檢查失敗時明確提示重試。
- Android 使用 APK 內建資源，不再註冊網頁更新用的 Service Worker；PWA 新版本下載完成後由使用者套用，首次安裝快取不強制重整。
- 小工具一次顯示天氣、匯率、翻譯、推薦 App 或更新維護其中一項；預設翻譯，並記住本次使用的分頁。既有網址錨點仍可直接開啟指定工具。
- 離開翻譯分頁、切到背景或離開頁面時停止相機；切換工具保留頁面內尚未送出的輸入內容。
- 全站「隨身資訊」可查看所選日期的票券、當晚住宿地址與歐洲緊急電話；支援地址複製、導航與電話連結。
- 票券改由使用者將 `.enc` 檔匯入裝置，檔案通過雜湊比對後才儲存於本機 IndexedDB；出示時仍需密碼。公開網站與 CI 產出的 APK 不附帶私人票券，請先在各台裝置匯入，原始檔保留在本機。
- 依目前隱私規則停止追蹤加密票券並遮蔽訂單 PIN；這取代下方歷史紀錄中將 `.enc` 上傳 Git 的做法，不改寫既有 Git 歷史。
- 首頁與每日頁新增「現在的安排／接下來」，依各地時區顯示行程；跨夜航班使用明確的出發與抵達時間。這是行程表提示，不代表即時位置或班次動態。
- 每日頁先呈現日期切換及下一步；當日簡介、地圖、交通細節與地點筆記可展開查看。點選地點筆記會自動展開所在區塊。
- 時間軸以獨立按鈕提供導航與票券，避免連結內嵌按鈕；憑證與彈性選項有文字標示。
- 全站手機頁首採用精簡的搜尋按鈕；每日頁沿用總覽、景點、交通、行李、小工具五項底部導覽。
- 滑動換頁避開按鈕、地圖與橫向選單，增加觸發距離；取消的觸控手勢不換頁。
- 手機與平板保留底部安全距離，支援鍵盤焦點提示與減少動態效果偏好。
- 維護時以 `node scripts/sync-version.cjs YYYYMMDD-NN` 同步所有頁面、快取版本及首頁同步標記。
