...
## EK 367 官方電子登機證整合與時間軸同步（2026-09-05）

- **第一段登機證已確認取得**：阿聯酋航空 EK 367（台北桃園 TPE T2 到杜拜 DXB T3，起飛 23:50）正式電子登機證已存入本機 `pdf/2026-09-06_23-50_EK367_TPE-DXB_boarding-pass.pdf`。
- **真實憑證加密入庫**：透過 AES-256-GCM 與 PBKDF2（600,000 次疊代）加密為 `site/assets/tickets/ek367-boarding-pass.enc`，並已納入 PWA Service Worker 離線預先快取清單。
- **登機時程與資訊精準核對**：
  - 劃位座位：`49C`（經濟艙 Economy，登機組別 Boarding Group 6）。
  - 報到序號：已完成線上報到；電子票號：`176-**********`；訂位代號：`FH****`。
  - 機場時限：行李托運截止時間為 `22:20`、安檢出境截止時間為 `22:50`、登機門開放為 `23:05`。
- **09/06 每日行程頁時間軸細緻化**：
  - `20:50 - 22:20`：抵達桃園機場 T2、阿聯酋航空報到與行李托運（托運截止 22:20）。
  - `22:20 - 22:50`：安全檢查與證照查驗（安檢截止 22:50）。
  - `23:05 - 23:50`：EK 367 登機（座位 49C），出示電子登機證 2D 條碼掃描登機。
  - `飛行中`：台北 TPE 至杜拜 DXB 跨夜飛行（約 8 小時 45 分，杜拜 04:35 抵達）。
- **第二段 EK 151 報到狀態說明**：第二段航班 EK 151（杜拜 08:20 到哥本哈根 13:15）因航空公司規定需於起飛前 48 小時內開放線上 Check-in，目前尚未到達開放時限；網站已註記開放規則，09/07 時間軸暫以全段官方電子機票作為備查憑證。
- **全站快取升級**：PWA 快取版本提升至 `eccv-guide-v20260905-02`，全站 19 份 HTML 檔案同步更新。
