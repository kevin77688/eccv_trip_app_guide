...
(async () => {
  ...
  try {
    for (const [name, width, height] of [['phone', 390, 844], ['desktop', 1280, 800]]) {
      ...
      await page.waitForFunction(() => document.querySelector('[data-ticket-availability]')?.textContent.includes('票券暫時無法載入'));
      await page.locator('[data-ticket-import-options] summary').click();
