...
for (const directory of [site, path.join(site, 'days')]) {
  for (const name of fs.readdirSync(directory).filter(name => name.endsWith('.html'))) {
    ...
    for (const module of ['ticket-store', 'essentials']) {
...
for (const relative of ['sw.js', 'js/core.js', 'js/pages/tools.js']) {
  ...
  for (const module of ['ticket-store', 'essentials']) if (relative === 'sw.js' && !text.includes(`'./js/${module}.js'`) && fs.existsSync(path.join(site, `js/${module}.js`))) text = text.replace("'./js/app.js',", `'./js/app.js',\n  './js/${module}.js',`);
