...
(async () => {
  ...
  try {
    for (const [name, width, native] of [['phone', 390, false], ['desktop', 1280, false], ['android-bridge', 390, true]]) {
      ...
      assert.deepEqual(loaded, { count: 11, allPresent: true });
    ...
    await context.close();
    console.log(JSON.stringify({ result: 'passed', screenshots: output, checks: 'all 11 bundled files, direct opening without import, offline persistence and authenticated decryption' }));
